...
// class Preproc {
//     public:
//         Preproc(po::variables_map& params, StateTransition& adpt5Tables, StateTransition&
//         adpt3Tables); ~Preproc();
...
//         std::size_t nibble(dtp::QualVector& qual, std::pair<std::size_t,std::size_t>& bnds); //
//         window trimming

//         // merging

//         void merging(std::string& fwdSeqid, auto& fwdSeq, auto& fwdQual, std::string& revSeqid,
//         auto& revSeq,
//                      auto& revQual, auto& outFwd, auto& outR1only, auto& outR2only, auto&
//                      outR1unmrg, auto& outR2unmrg);
//         std::pair<dtp::DNAVector, dtp::QualVector> mergeReads(dtp::DNASpan& fwdSeq,
