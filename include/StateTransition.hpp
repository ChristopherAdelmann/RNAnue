...
class StateTransition {
   public:
    ...
    std::size_t calcReadPos(const dtp::DNAVector& seq, const dtp::Left& left,
                            const dtp::Right& right);
    int addState(std::vector<dtp::State>& states, dtp::State state,
                 std::vector<dtp::State>::size_type& size);
    int transition(dtp::DNAVector seq, dtp::DNAVector suffix, std::size_t readPos, dtp::Left& left,
                   dtp::Right& right);
