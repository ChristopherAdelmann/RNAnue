...
Analysis::Analysis(po::variables_map _params) : params(_params)
{
    ...
    while (getline(anno, line))
    {
        ...
        if (features.count(tokens[0]) == 0)
        { // check of RefName is in map
...
void Analysis::start(pt::ptree sample)
{
    ...
    outInts << "#QNAME\tSegment1Strand\tSegment1Start\tSegment1End\tSegment1RefName\tSegment1Name\tSegment1AnnoStrand\tSegment1Product\tSegment1Orientation\tSegment2Strand\tSegment2Start\tSegment2End\tSegment2RefName\tSegment2Name\tSegment2AnnoStrand\tSegment2Product\tSegment2Orientation\tenergy\tcomplementarity\n";
    ...
    for (auto &&chimericReadPair : fin | seqan3::views::chunk(2))
    {
        ...
        for (auto &read : chimericReadPair)
        {
            ...
            if (features.count(refID) > 0)
            {
                ...
                {
                    if ((start >= featuresSubset[i].first.first && start <= featuresSubset[i].first.second) ||
                        (end >= featuresSubset[i].first.first && end <= featuresSubset[i].first.second))
                    {
                        ...
                        annoStrand = retrieveTagValue(featuresSubset[i].second, "strand", annoStrand);
                ...
                if (overlaps > 1)
                {
                    std::cout << "overlaps: " << overlaps << " " << refID << ":" << start << "-" << end << std::endl;
                    ambiguousOverlaps++;
                }
                else if (overlaps == 1)
                {
    ...
    std::cout << "unique overlaps: " << uniqueOverlaps << "ambigous overlaps:" << ambiguousOverlaps << std::endl;
