        ...
        for (unsigned i = 0; i < splits.size(); ++i) {
          for (unsigned j = i + 1; j < splits.size(); ++j) {
            ...
            if (true)  // splitSegments.empty()
            {
              ...
              if (!cmpl.a.empty() && hyb.has_value()) {  // check that there is data for
                ...
                if (cmpl.cmpl > filters.first) {  // complementarity is higher
                  ...
                  if (cmpl.cmpl == filters.first) {
                    ...
                    if (hybEnergy == filters.second) {  // same cmpl & hyb -> ambigious read!
                      addComplementarityToSamRecord(splits[i], splits[j], cmpl);
                      addHybEnergyToSamRecord(splits[i], splits[j], hyb.value());
                      splitSegments.push_back(std::make_pair(splits[i], splits[j]));
                    }
...
std::optional<InteractionWindow> SplitReadCalling::getContinuosNucleotideWindows(
  ...
  if (forwardPair.first + 1 != forwardPair.second || reversePair.first - 1 != reversePair.second) {
    return std::nullopt;
  }
  ...
  if (firstPairInSeq1 && secondPairInSeq1) {
    return InteractionWindow{seqan3::dna5_vector{seq1[forwardPair.first], seq1[forwardPair.second]},
  ...
  // Due to sorting of base pairs first pair in second sequence and second pair in first sequence is
  // not possible
  return std::nullopt;
}
