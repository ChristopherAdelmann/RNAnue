...
// Preproc::Preproc(po::variables_map& params, StateTransition& adpt5Tables, StateTransition&
// adpt3Tables)
...
//                 std::pair<dtp::DNAVector, dtp::QualVector> merged = mergeReads(trmSeqFwd,
//                 trmQualFwd,
//                                                                                trmSeqRev,
//                                                                                trmQualRev);
...
//                         auto revSeqRC = trmSeqFwd | std::views::reverse |
//                         seqan3::views::complement; auto revQualR = trmQualFwd |
//                         std::views::reverse; dtp::DNAVector revSeqRCVec{revSeqRC.begin(),
//                         revSeqRC.end()}; dtp::QualVector revQualRVec{revQualR.begin(),
//                         revQualR.end()}; dtp::FASTQRecord outR2onlyObj{seqidFwd, revSeqRCVec,
//                         revQualRVec};
//                         {
//                             std::lock_guard<std::mutex> lock(outMutex);
...
//             dtp::DNAVector bases = adpt5Tables.getBases().find(std::make_pair(tab.first.first,
//             tab.first.second))->second; fndPos = boyermoore(seq, tab.second, bases,
//             tab.first.second); if (fndPos != std::string::npos) { boundaries.first = fndPos +
//             bases.size()-1; }
//         }
//     }
...
//             bases = adpt3Tables.getBases().find(std::make_pair(tab.first.first,
//             tab.first.second))->second; fndPos = boyermoore(seq, tab.second, bases,
//             tab.first.second); if (fndPos != std::string::npos) { boundaries.second = fndPos; }
//         }
//     }
//     return boundaries;
// }
...
// //
// std::pair<dtp::DNAVector, dtp::QualVector> Preproc::mergeReads(dtp::DNASpan& fwdSeq,
// dtp::QualSpan& fwdQual,
//                                                                dtp::DNASpan& revSeq,
//                                                                dtp::QualSpan& revQual) {
...
//     /* convert to String - this seems counterintuitive, but working of views on DNAVectors cannot
//     be concatenated
...
//         std::string mrgSeqStr = fwdSeqStr.substr(0, fwdLcsPos+lcs.size()) +
//         revSeqRCStr.substr(0,revLcsPos); std::string mrgQualStr = fwdQualStr.substr(0,
//         fwdLcsPos+lcs.size()) + revQualRCStr.substr(0,revLcsPos);
