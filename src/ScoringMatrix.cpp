        ...
        for (unsigned i = 1; i < height; ++i) {
            for (unsigned j = 1; j < width; ++j) {
                ...
                if (scores) {  // memory allocation for scores successful
                    scores[0] = *(scoring + ((i - 1) * width) + (j - 1)) +
                                scoringScheme[a[i - 1]][b[j - 1]];
                    scores[1] = *(scoring + ((i - 1) * width) + j) - gapCost;
                    scores[2] = *(scoring + (i * width) + (j - 1)) - gapCost;

                    /*
                    std::cout << "scores / compare: " << a[i-1] << " and " << b[j-1] <<
                    std::endl; std::cout << "similarity: " << scoringScheme[a[i-1]][b[j-1]] <<
                    std::endl; std::cout << scores[0] << std::endl; std::cout << scores[1] <<
                    std::endl; std::cout << scores[2] << std::endl; std::cout << sc.second <<
                    std::endl;
                    */
