...
void SeqRickshaw::processPairedEnd(pt::ptree sample)
{
    ...
    std::vector<SeqRickshaw::Adapter> adapters5f = loadAdapters(adpt5f, SeqRickshaw::TrimConfig::Mode::FIVE_PRIME);
    std::vector<SeqRickshaw::Adapter> adapters3f = loadAdapters(adpt3f, SeqRickshaw::TrimConfig::Mode::THREE_PRIME);
    std::vector<SeqRickshaw::Adapter> adapters5r = loadAdapters(adpt5r, SeqRickshaw::TrimConfig::Mode::FIVE_PRIME);
    std::vector<SeqRickshaw::Adapter> adapters3r = loadAdapters(adpt3r, SeqRickshaw::TrimConfig::Mode::THREE_PRIME);
 ...
 *
 * This function loads adapters from either a file or a sequence and returns them as a vector of SeqRickshaw::Adapter objects.
 *
 ...
 */
std::vector<SeqRickshaw::Adapter> SeqRickshaw::loadAdapters(std::string const &filenameOrSequence, const SeqRickshaw::TrimConfig::Mode trimmingMode)
{
    std::vector<SeqRickshaw::Adapter> adapters;

    if (filenameOrSequence.empty())
        return adapters;
    ...
    {
        seqan3::dna5_vector adapterSequence = filenameOrSequence | seqan3::views::char_to<seqan3::dna5> | seqan3::ranges::to<std::vector>();
        adapters.push_back(SeqRickshaw::Adapter{adapterSequence, missmatchRateTrim, trimmingMode});
    }

    for (auto &adapter : adapters)
    {
        Logger::log(LogLevel::INFO, "Loaded adapter: " + filenameOrSequence + " with " + std::to_string(adapter.maxMissmatches) + " allowed missmatches.");
    }
...
void SeqRickshaw::trimWindowedQuality(record_type &record)
{
    ...
    {
        auto windowQual = record.base_qualities() | seqan3::views::slice(trimmingEnd - windowTrimSize, trimmingEnd);
    ...
    record.base_qualities().erase(record.base_qualities().begin() + trimmingEnd, record.base_qualities().end());
}
 ...
 */
template <typename record_type>
std::optional<record_type> SeqRickshaw::mergeRecordPair(record_type &record1, record_type &record2)
{
...
void SeqRickshaw::trimAdapter(const SeqRickshaw::Adapter &adapter, auto &record)
{
    ...
    for (auto const &result : seqan3::align_pairwise(std::tie(adapter.sequence, seq), alignment_config))
    {
 ...
 */
seqan3::align_cfg::method_global SeqRickshaw::TrimConfig::alignmentConfigFor(SeqRickshaw::TrimConfig::Mode mode)
{
...
void SeqRickshaw::processSingleEndRecordChunk(SeqRickshaw::SingleEndFastqChunk &chunk, const std::vector<SeqRickshaw::Adapter> &adapters5, const std::vector<SeqRickshaw::Adapter> &adapters3)
{
    auto it = std::remove_if(chunk.records.begin(), chunk.records.end(), [&](auto &record)
        ...
        if (windowTrimSize > 0)
            trimWindowedQuality(record);

        for (auto const& adapter : adapters5)
            trimAdapter(adapter, record);
