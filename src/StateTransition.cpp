...
dtp::Tables StateTransition::getTables() { return tables; }

dtp::Bases StateTransition::getBases() { return bases; }
...
dtp::Tables StateTransition::preprocessing(std::string type, std::string adptFile) {
    ...
    for (auto& rec : adptSeqFile) {
        ...
        bases.insert(std::make_pair(std::make_pair(rec.id(), rec.sequence()),
                                    seqIO::determineAlphabet(rec.sequence())));
    }
    return adptTables;
}
    ...
    for (std::vector<dtp::State>::size_type i = 0; i < statesSize; ++i) {
        ...
        for (unsigned j = 0; j < alphabet.size(); ++j) {  // iterate through any other possibility
            ...
            if (seq[readPos] == alphabet[j]) {  // match
                if (right.first == std::string::npos &&
                    right.second == std::string::npos) {  // right block is empty
                ...
                if (std::count(nextState.begin(), nextState.end(), 'X') != seq.size()) {
                    nextStateID = addState(
                        states, std::make_tuple(nextState, nextLeft, nextRight, nextReadPos),
                        statesSize);
                    lookup.insert(std::make_pair(std::make_pair(i, alphabet[j]),
                                                 std::make_tuple(0, nextStateID, nextReadPos, 0)));
                } else {
                    lookup.insert(
                        std::make_pair(std::pair(i, alphabet[j]), std::make_tuple(0, 0, 0, 1)));
                }
                ...
                if (right.first == std::string::npos && right.second == std::string::npos) {
                    ...
                    suffix = seqIO::spanToVector(seqan3::views::slice(
                        seq, right.first, right.first + (right.second - right.first) + 1));
                ...
                nextReadPos = calcReadPos(seq, nextLeft, nextRight);
                nextStateID =
                    addState(states, std::make_tuple(nextState, nextLeft, nextRight, nextReadPos),
                             statesSize);
                lookup.insert(std::make_pair(std::make_pair(i, alphabet[j]),
                                             std::make_tuple(shift, nextStateID, nextReadPos, 0)));
            }
        }
    }
    return lookup;
...
std::size_t StateTransition::calcReadPos(const dtp::DNAVector& seq, const dtp::Left& left,
                                         const dtp::Right& right) {
    std::size_t readPos{std::string::npos};
...
int StateTransition::addState(std::vector<dtp::State>& states, dtp::State state,
                              std::vector<dtp::State>::size_type& size) {
    int pos = -1;
...
int StateTransition::transition(dtp::DNAVector seq, dtp::DNAVector suffix, std::size_t readPos,
                                dtp::Left& left, dtp::Right& right) {
    int shift = 0;
