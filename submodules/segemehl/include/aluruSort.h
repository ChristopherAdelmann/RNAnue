...
#define BUCKETRET(X, N, I, B)      \
    for (B = 0; B < (N); B++) {    \
        if (X[B].id == (I)) break; \
    }

#define BUCKETINIT(X, I) \
    (X).id = (I);        \
    (X).elems = NULL;    \
    (X).noofelems = 0;   \
    (X).allocsize = 0
...
void sortAluruSubstrings(void *space, vector_t **qdlist, Uint nooflists, Alurubucket *bckts,
                         Uint noofbckts, Uint *R, char *cl, Uint len, char Q);
