...
void bl_fastaSetSequence(void *space, fasta_t *f, Uint elem, char *seq,
                         Uint len) {
  Uint oldlen, newlen;
  char *oldseq;
...
Uint bl_fastaSoftClip(void *space, fasta_t *f, Uint elem, char *p5, Uint p5len,
                      Uint p5scr, char *p3, Uint p3len, Uint p3acc,
                      Uint pAlen) {
  Uint len, l = 0, r = 0, k;
  char *seq;
  ...
  // fprintf(stderr, "***************** clipping '%s' - '%s' of length %d -
  // %d\n", p5, p3, p5len, p3len);
...
Uint bl_fastaMateSoftClip(void *space, fasta_t *f, Uint elem, char *p5,
                          Uint p5len, Uint p5scr, char *p3, Uint p3len,
                          Uint p3acc, Uint pAlen) {
  Uint len, l = 0, r = 0, k;
  char *seq;
...
Uint bl_fastaMateHardClip(void *space, fasta_t *f, Uint elem, Uint p5,
                          Uint p3) {
  Uint k, len;
...
void bl_fastaSetQuality(void *space, fasta_t *f, Uint elem, char *qlty,
                        Uint len) {
  Uint oldlen, newlen;
  char *oldqlty;
...
void bl_fastaAdd(void *space, fasta_t *f, char *descr, Uint descrlen,
...
void bl_fastxAdd(void *space, fasta_t *f, char *descr, Uint descrlen,
                 char *sequence, char *quality, Uint seqlen, Uint n) {
  assert(n == f->active_noofseqs);
...
unsigned char bl_fastaCheckMateID(fasta_t *f, Uint elem, char *mateid,
                                  Uint matelen) {
...
void bl_fastaAddMate(void *space, fasta_t *f, char *descr, Uint descrlen,
                     char *sequence, Uint seqlen, Uint n) {
...
void bl_fastxAddMate(void *space, fasta_t *f, char *descr, Uint descrlen,
                     char *sequence, char *quality, Uint seqlen, Uint n) {
  bl_fastaSetMateDescription(f, n, descr, descrlen);
  if (f->checkid) {
...
fasta_t *bl_fastxCopy(void *space, fasta_t *f, Uint offset, Uint size) {
  ...
  if (bl_fastaHasQuality(f)) {
    copy->quals =
        ALLOCMEMORY(space, NULL, CharSequence *, copy->active_noofseqs);
  }
  ...
  for (j = 0; j < copy->active_noofseqs; j++) {
    ...
    copy->minlen = (f->seqs[j + offset]->length < copy->minlen)
                       ? f->seqs[j + offset]->length
                       : copy->minlen;
    copy->maxlen = (f->seqs[j + offset]->length > copy->maxlen)
                       ? f->seqs[j + offset]->length
                       : copy->maxlen;
  }
...
fasta_t *bl_fastxCopyIndex(void *space, fasta_t *f, Uint offset, Uint size) {
  ...
  chunkindex = ALLOCMEMORY(space, NULL, fastxseqindex_t, 1);
  chunkindex->ap = ALLOCMEMORY(space, NULL, seqaccesspoint_t, chunks);
  memmove(chunkindex->ap, &f->chunkindex->ap[offset],
          chunks * sizeof(seqaccesspoint_t));
  chunkindex->size = chunks;
  chunkindex->allocated = chunks;
...
Uint bl_fastxScan(void *space, char *filename, struct access *index,
                  off_t offset, fastxfileindex_t *findex, Uint max,
                  Uint *minlen, Uint *maxlen, unsigned char *minq,
                  unsigned char *maxq) {
  char ch;
  char idchar = 0;
  int ret = 0;
  ...
  if (findex->size + 2 >= findex->allocated) {
    findex->ap = ALLOCMEMORY(space, findex->ap, seqaccesspoint_t,
                             findex->allocated + 11);
    findex->allocated += 11;
  }
...
fastxseqindex_t *bl_fastxChunkIndex(void *space, char **filenames,
                                    struct access **gzindex,
                                    fastxfileindex_t **findex, Uint *n,
                                    Uint nooffiles, Uint total, Uint k) {
  ...
  for (j = 0; j < nooffiles; j++) {
    ...
    for (i = 1; i < chunks; i++) {
      ...
      //   fprintf(stderr, "chunk %d in file %d (curchunksize %d): skiping %d (=
      //   nseqsfile:%d
      //   - findex.noofseqs:%d)\n",
      //       i, j, curchunksize, skip, nseqsfile,
      //       findex[j]->ap[cur].noofseqs);
      ...
      if (skip) {
        if (gzindex) {
          bl_fastxScan(space, filenames[j], gzindex[j],
                       findex[j]->ap[cur].offset, tmp, skip, &minlen, &maxlen,
                       &maxqual, &minqual);
        } else {
          bl_fastxScan(space, filenames[j], NULL, findex[j]->ap[cur].offset,
                       tmp, skip, &minlen, &maxlen, &maxqual, &minqual);
        }
      } else {
        if (tmp->size + 2 >= tmp->allocated) {
          tmp->ap = ALLOCMEMORY(space, tmp->ap, seqaccesspoint_t,
                                tmp->allocated + 11);
          tmp->allocated += 11;
        }
...
int bl_fastxGetChunk(fasta_t *fasta, Uint k) {
  ...
  while (i < chunks && fasta->chunkindex->ap[i].cumnoofseqs <= k)
    i++;
  while (i > 0 && fasta->chunkindex->ap[i - 1].cumnoofseqs > k)
    i--;
  ...
  if (fasta->chunkindex->ap[i].cumnoofseqs <= k ||
    ...
    DBG("list: chunks:%d, i:%d, idx[i]:%d, idx[i-1]:%d, k:%d, fid:%d\n", chunks,
        i, fasta->chunkindex->ap[i].cumnoofseqs, 0, k,
...
Uint bl_fastxGetChunkElem(void *space, fasta_t *f, Uint k) {
  ...
  if (f->chunkIsActive) {
    if (f->chunkindex->ap[cur].cumnoofseqs > k &&
      ...
      if (cur > 0)
        off = f->chunkindex->ap[cur - 1].cumnoofseqs;
      assert(k >= off);
      return k - off;
    }
  ...
  if (f->gzip) {
    f = bl_fastxgzRead(space, f, f->filenames[fid], f->gzindex[fid], f->upper,
                       f->lower, f->chunkindex->ap[cur].offset, 0,
    ...
    f = bl_fastxRead(space, f, f->filenames[fid], f->upper, f->lower,
                     f->chunkindex->ap[cur].offset, 0,
                     f->chunkindex->ap[cur].noofseqs,
                     // f->gzip, f->index[fid],
                     bl_fastxAdd);
    if (f->hasMates) {
...
fasta_t *bl_fastxIndex(void *space, fasta_t *set, char **filenames,
                       Uint nooffiles, unsigned char isMate, unsigned char gzip,
                       Uint pieces) {
  int i, len = 0;
  Uint total = 0, *ftotal = NULL;
  Uint chunksize = 10000, minlen = 0, maxlen = 0, seqsperpiece, noofchunks,
       rest;
  ...
  for (i = 0; i < nooffiles; i++) {
    ...
    if (gzip)
      ftotal[i] = bl_fastxScan(space, filenames[i], gzindex[i], 0, findex[i], 0,
                               &minlen, &maxlen, &minqual, &maxqual);
    else
      ftotal[i] = bl_fastxScan(space, filenames[i], NULL, 0, findex[i], 0,
                               &minlen, &maxlen, &minqual, &maxqual);
  ...
  chunkindex = bl_fastxChunkIndex(space, filenames, gzindex, findex, ftotal,
                                  nooffiles, total, chunksize);
...
fasta_t *bl_fastxRead(void *space, fasta_t *fasta, char *filename,
                      unsigned char upper, unsigned char lower, off_t offset,
                      Uint startseq, Uint lastseq,
                      // unsigned char gzip, struct access *index,
                      void (*handler)(void *, fasta_t *, char *, Uint, char *,
                                      char *, Uint, Uint)) {
  FILE *fp;
  char ch;
  char *buffer;
  ...
  while ((ch = (gzip) ? bl_getgzidxc(gzf) : getc(fp)) != EOF) {
    ...
    if (ch == idchar &&
      ...
      handler(space, fasta, descrbuffer, descrlength, seqbuffer, qualbuffer,
              len, n);
      n++;
  ...
  if ((!fastq && len > 0) || (qual && len > 0 && len == seqlen)) {
    ...
    handler(space, fasta, descrbuffer, descrlength, seqbuffer, qualbuffer, len,
            n);
  }
...
fasta_t *bl_fastxgzRead(void *space, fasta_t *fasta, char *filename,
                        struct access *idx, unsigned char upper,
                        unsigned char lower, off_t offset, Uint startseq,
                        Uint lastseq,
                        void (*handler)(void *, fasta_t *, char *, Uint, char *,
                                        char *, Uint, Uint)) {
  FILE *fp = NULL;
...
fasta_t *bl_fastxGetSet(void *space, char **filenames, unsigned int nooffiles,
  ...
  if (!index) {
    ...
    for (i = 0; i < nooffiles; i++) {
      if (!gzip) {
        set = bl_fastxRead(space, set, filenames[i], upper, lower, 0, n,
                           0, // gzip, gzindex,
                           bl_fastxAdd);
        ...
        set = bl_fastxgzRead(space, set, filenames[i], gzindex, upper, lower, 0,
                             n, 0, bl_fastxAdd);
...
fasta_t *bl_fastxGetMateSet(void *space, fasta_t *set, char **filenames,
                            unsigned int nooffiles, unsigned char upper,
                            unsigned lower, unsigned char index, Uint pieces) {
  Uint i, prefixlen, n = 0;
  unsigned char gzip = 2;
  struct access *gzindex;
  ...
  if (!index) {
    ...
    for (i = 0; i < nooffiles; i++) {
      if (!gzip) {
        set = bl_fastxRead(space, set, filenames[i], upper, lower, 0, n, 0,
                           bl_fastxAddMate);
...
void bl_fastxDump(void *space, fasta_t *fasta, char *desc, Uint desclen,
                  char *sequence, char *quality, Uint quallen, Uint n) {
  fprintf(stderr, "%s\n", desc);
  fprintf(stderr, "%s\n", sequence);
  fprintf(stderr, "+%s\n", &desc[1]);
  fprintf(stderr, "%s\n", quality);
...
void bl_GFFwrite(char *filename, annotationtrack_t *set) {
  ...
  for (i = 0; i < set->noofitems; i++) {
    ...
    fprintf(fp, "%" PRIu64 "\t%" PRIu64 "\t%c\t", g->start + 1, g->end + 1,
            g->strand);
...
void bl_GFFAddAttribute(void *space, annotationitem_t *item, char *attr,
                        Uint len) {
  item->attributes =
      ALLOCMEMORY(space, item->attributes, char *, item->noofattributes + 1);

  item->attributelen =
      ALLOCMEMORY(space, item->attributelen, Uint, item->noofattributes + 1);
