...
void bl_fileBinsDump(bl_fileBins_t *fb) {
    Uint i;
    ...
    fprintf(stdout, "total number of filebins: %d\n", fb->noofbins);
    for (i = 0; i < fb->noofbins; i++) {
        if (i > 0) fprintf(stdout, ";");
        fprintf(stdout, "%d\t%s\t%s\t%lld\t%lld", i, fb->b[i].fname, fb->b[i].id->classname,
                fb->b[i].id->start, fb->b[i].id->end);
    }
    return;
}
...
void bl_fileBinsDomainDump(bl_fileBinDomains_t *dms) {
    Uint i;

    fprintf(stdout, "total number of domains: %d\n", dms->noofdomains);
    for (i = 0; i < dms->noofdomains; i++) {
        fprintf(stdout, "%d\t%s\t%d\t%d\t", i, dms->domain[i].domainname, dms->domain[i].domainsize,
                dms->domain[i].bins.noofbins);
        bl_fileBinsDump(&dms->domain[i].bins);
    }
    fprintf(stdout, "\n");
...
void bl_fileBinsGetInfo(bl_fileBins_t *fb) {
    Uint i;
    ...
    for (i = 0; i < fb->noofbins; i++) {
        NFO("Bin[%d] %s (classname: %s, range:%lld-%lld)\n", i, fb->b[i].fname,
            fb->b[i].id->classname, fb->b[i].id->start, fb->b[i].id->end);
    }
    return;
}
...
void bl_fileBinsDomainGetInfo(bl_fileBinDomains_t *dms) {
    Uint i;

    NFO("total number of domains: %d\n", dms->noofdomains);
    for (i = 0; i < dms->noofdomains; i++) {
        NFO("Domain[%d] %s, domainsize: %d\n", i, dms->domain[i].domainname,
            dms->domain[i].domainsize);
...
void bl_fileBinsWriteLn(void *space, bl_fileBin_t *fx, char *line) {
    fprintf(fx->fp, "%s\n", line);
    fx->lines++;
    return;
}
...
unsigned char bl_fileBinsIsOpen(bl_fileBin_t *fx) { return (fx->fp != NULL); }
...
int bl_fileBinsClose(bl_fileBin_t *fx) {
    int ret;
...
void bl_fileBinsUnlock(bl_fileBin_t *bin) {
    int ret;
    assert(bin);
...
void bl_fileBinsLock(bl_fileBin_t *bin) {
    assert(bin);
...
FILE *bl_fileBinsOpen(void *space, bl_fileBin_t *bin, const char *mode) {
    /*empty files?*/
    ...
    setvbuf(bin->fp, NULL, _IOFBF, 524288);
...
bl_fileBin_t *bl_fileBinsFind(void *space, bl_fileBins_t *bins,
...
void bl_fileBinsInit(void *space, bl_fileBins_t *bins) {
    bins->b = NULL;
    bins->noofbins = 0;
...
Uint bl_fileBinsDomainsGetList(void *space, bl_fileBinDomains_t *domains, char **domainnames[],
                               Uint **domainsizes) {
    Uint i = 0, k = 0;
    char **list = NULL;
    Uint *ll = NULL;
    ...
    for (i = 0; i < domains->noofdomains; i++) {
        if (list == NULL || list[k - 1] != domains->domain[i].domainname) {
...
/*-------------------- bl_fileBinsDomainsGetNoOfNames ------------------
 *
 * @brief pass list of k domain names and their length
 * @author Steve Hoffmann
 *
...
Uint bl_fileBinsDomainsGetNoOfNames(void *space, bl_fileBinDomains_t *domains) {
    Uint i = 0, k = 0;
    char **list = NULL;
...
void bl_fileBinsCloseAll(bl_fileBins_t *bins) {
    Uint i;
    assert(bins);
    for (i = 0; i < bins->noofbins; i++) {
...
void bl_fileBinsAdd(void *space, bl_fileBins_t *bins, Uint add,
                    bl_fileBinClass_t *(*assigner)(void *, int, void *), void *nfo,
                    char **filenames, char *temp, Uint tmplen) {
    Uint i;
    char *fname;
...
void bl_fileBinsDestruct(void *space, bl_fileBins_t *bins) {
    Uint i;
...
void bl_fileBinDomainsDestruct(void *space, bl_fileBinDomains_t *dms) {
    Uint i;
...
void bl_fileBinDomainsCloseAll(bl_fileBinDomains_t *dms) {
    Uint i;
    assert(dms->domain);
...
bl_fileBinDomains_t *bl_fileBinsDomainsInit(void *space, char **domainnames, Uint *domainsizes,
                                            Uint noofdomains, Uint totalsize, Uint avgbins,
                                            Uint maxbins, char *filenametemplate, Uint tmplen) {
    Uint i, j, noofbins, e = 0, binsize, maxbinperdomain, maxdomainsize = 0, est = 0;
    ...
    for (i = 0; i < noofdomains; i++) {
        ...
        bl_fileBinsAdd(space, &dms->domain[i].bins, noofbins, NULL, NULL, NULL, filenametemplate,
                       tmplen);
...
bl_fileBin_t *bl_fileBinsDomainGetBin(bl_fileBinDomains_t *dms, char *domainname, Uint pos) {
    Uint i;

    if (domainname == NULL) {
        return &dms->domain[0].bins.b[0];
    }
    ...
    if (i == dms->noofdomains) return NULL;
...
void bl_fileBinsUnixSort(void *space, bl_fileBins_t *fb, const char *fldstr, const char delim) {
    Uint i;
    // int ret;
    char *filename;
...
void bl_fileBinDomainsUnixSort(void *space, bl_fileBinDomains_t *dms, const char *fldstr,
                               const char delim) {
    Uint i, j;
    // int ret;
    char *filename;
...
void bl_fileBinDomainsSortMerge(void *space, bl_fileBinDomains_t *dms, char *bname, Uint bnamelen,
                                char *suf, Uint suflen, const char *fldstr, const char delim,
...
void bl_fileBinDomainsMerge(void *space, bl_fileBinDomains_t *dms, char *bname, Uint bnamelen,
                            char *suf, Uint suflen, char **header, unsigned char remove) {
    FILE *outfile = NULL, *fp;
    size_t buffersize = 1024, len;
    off_t fs;
    char *buffer, *cname, *ccname, *newname = NULL;
    Uint i, j, cnamelen;
    ...
    for (i = 0; i < dms->noofdomains; i++) {
        ...
        if (strlen(ccname) == 0) {
            FREEMEMORY(NULL, ccname);
            bl_bsprintf(&ccname, "emptydomain");
        }

        bl_bsprintf(&newname, "%s_%s.%s", bname, ccname, suf);
...
void bl_fileBinsSortLine(void *space, bl_fileBins_t *bins, unsigned char fileSort, char *filename,
                         unsigned char remove, LLint (*keyaccess)(char *, void *), void *nfo) {
    Uint i, k, j = 0, len;
    bl_fileSort_t *data;
    LLint res;
    ...
    FILE *outfile;
    struct stat st;
    char *temp = "filebinsort";

    if (filename) {
        outfile = fopen(filename, "w");
    } else {
        tmpname = bl_getTempFile(".", temp);
...
void bl_fileBinsMerge(void *space, char *filename, bl_fileBins_t *bins, unsigned char delete) {
    FILE *outfile, *fp;
    char *line;
    Uint i, len;
...
int bl_fileBinsCClassSelect(void *id, void *nfo) {
...
void bl_fileBinsCClassRename(void *space, bl_fileBins_t *fb, char *bname, Uint bnamelen, char *suf,
                             Uint suflen) {
...
bl_fileBinClass_t *bl_fileBinCClassAssign(void *space, int id, void *nfo) {
    bl_fileBinClass_t *ptr;
    char **classnames;
