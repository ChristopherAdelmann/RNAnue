 ...
 *  Url: $URL:
 * http://www.bioinf.uni-leipzig.de/svn/segemehl/segemehl/branches/esa/trunk/libs/kdseed.c
 * $
 *
 */
...
inline int kdscore(branch_t *b) {
  if (b->mat < b->mis + b->ins + b->del)
    return 0;
...
int kd_getBranchScore(matchstem_t *M, Uint i, Uint q) {
  int scr;
...
double kd_getBranchEvalue(matchstem_t *stem, Uint i, Uint q, Uint m, Uint n,
                          karlin_t *stats) {
  int scr;
  double space;
...
inline void matchstemAddBranch(void *space, matchstem_t *a, Uint mat, Uint q,
                               Uint mis, Uint ins, Uint del, Uint l, Uint r,
                               Uint u, Uint v) {
...
branch_t *copyBranch(void *space, branch_t *a) {
  branch_t *ret = NULL;
...
inline void matchstemModifyBranch(void *space, matchstem_t *a, Uint k, Uint mat,
                                  Uint q, Uint mis, Uint ins, Uint del, Uint l,
                                  Uint r, Uint u, Uint v) {
...
inline void pushkdlcp(void *space, VStack *vstack, char *p, Uint m, Uint kp,
                      kdiffm_t *data) {
  Uint i;
...
inline void pushkdbranchesArr(void *space, Suffixarray *s, VStack *vstack,
                              char *p, Uint m, Uint kp, kdiffm_t *data,
                              PairUint pr) {
  ...
  for (v = 0; v < count; v++) {
    ...
    if (l <= r) {
      if (l != child.a || r != child.b) {
        ...
        if (isallowedIUPAC() &&
            matchIUPAC(p[data->qptr], s->seq->sequences[s->suftab[l] + lcp])) {
          bl_vstackPush(vstack, data);
          kdiffm_t *tmp = (kdiffm_t *)bl_vstackTop(vstack);
          tmp->qptr++;
          tmp->sptr++;
...
inline void pushkdbranches(void *space, Suffixarray *s, VStack *vstack, char *p,
                           Uint m, Uint kp, kdiffm_t *data, PairUint pr) {
  Uint l, r, v, i, lcp = 0;
  char cur;
  Container *c;
  PairUint child;
...
inline void kd_updatebranches(matchstem_t *b, kdiffm_t *data,
                              unsigned int seedlen) {
  if (data->mis + data->ins + data->del == 0 && data->mat <= seedlen) {
    matchstemModifyBranch(NULL, b, 0, data->mat, data->mat, data->mis,
                          data->ins, data->del, data->child.a, data->child.b,
                          data->parent.a, data->parent.b);
  }
  if (data->mat > data->ins + data->mis + data->del) {
    if (b->noofbranches == 1 ||
        (int)data->mat - (int)data->ins - (int)data->mis - (int)data->del >
            kdscore(&b->branches[1])) {
      b->branches = realloc(b->branches, sizeof(branch_t) * 2);
      b->noofbranches = 2;
      matchstemModifyBranch(NULL, b, 1, data->mat, data->mat, data->mis,
                            data->ins, data->del, data->child.a, data->child.b,
                            data->parent.a, data->parent.b);
    } else {
      if ((int)data->mat - (int)data->ins - (int)data->mis - (int)data->del ==
          kdscore(&b->branches[1])) {
        ...
        if (i == b->noofbranches)*/
        matchstemAddBranch(NULL, b, data->mat, data->mat, data->mis, data->ins,
                           data->del, data->child.a, data->child.b,
                           data->parent.a, data->parent.b);
      }
    }
  }
...
inline int kd_matchlcp(void *space, Suffixarray *s, char *p, unsigned int m,
                       kdiffm_t *data, unsigned int cursufpos,
                       unsigned int lcplen, unsigned int seedlen,
                       VStack *vstack, unsigned int maxdiff, int sext, int pmis,
                       int xoff, matchstem_t *b) {
  ...
  for (i = 0;
       i < lcplen && cursufpos + data->sptr + i < seqlen && data->qptr + i < m;
       i++) {
  ...
  if (pnlty > xoff) {
    ...
    if (lastmis + data->ins + data->del != 0) {
      if (lastmat > lastmis + data->ins + data->del && lastmat > origmat) {
        if (b->noofbranches == 1 ||
            (int)lastmat - (int)lastmis - (int)data->ins - (int)data->del >
                kdscore(&b->branches[1])) {
          b->branches = realloc(b->branches, sizeof(branch_t) * 2);
          b->noofbranches = 2;
          matchstemModifyBranch(space, b, 1, lastmat, lastmat, lastmis,
                                data->ins, data->del, data->child.a,
                                data->child.b, data->parent.a, data->parent.b);
        } else {
          if ((int)lastmat - (int)lastmis - (int)data->ins - (int)data->del ==
              kdscore(&b->branches[1])) {
            // CHANGED: do not insert equal branches,
            // can occur due to 'alignment' ambiguity
            //--> performance needs to be tested (tradeoff
            ...
            /*
               ...
               if (i == b->noofbranches)*/
            matchstemAddBranch(space, b, lastmat, lastmat, lastmis, data->ins,
                               data->del, data->child.a, data->child.b,
                               data->parent.a, data->parent.b);
          }
        }
  ...
  // and at least one more character was matched
  // compared to begin of function
  if (data->mis + data->ins + data->del != 0 && data->mat > origmat) {
    kd_updatebranches(b, data, seedlen);
  }
...
inline matchstem_t *kd_match(void *space, Suffixarray *s, char *p, Uint m,
                             Uint sext, Uint pmis, Uint xoff, Uint maxdiff,
                             Uint iprime, Uint jprime, Uint kprime, Uint lprime,
                             Uint sptr, Uint qptr) {
  Uint lcplen = 0, cursufpos = 0, seedlen = 10000;
  VStack *vstack;
  PairUint birth;
  matchstem_t *b;
  kdiffm_t data, *tmp;
  ...
  data.parent.a = iprime;
  data.parent.b = jprime;

  if (data.l == data.sptr && data.l < m && iprime != jprime) {
    data.child = getCharIntervalArr(space, s, iprime, jprime, 0, p[data.qptr]);

    if (data.kcnt < maxdiff || (isallowedIUPAC() && data.kcnt == maxdiff &&
                                couldMatchIUPAC(p[data.qptr]))) {
      pushkdbranches(space, s, vstack, p, m, maxdiff, &data, data.parent);
    }
  }

  if (data.child.a > data.child.b) {
    matchstemAddBranch(space, b, data.mat, data.mat, data.mis, data.ins,
                       data.del, iprime, jprime, data.parent.a, data.parent.b);
  } else {
    data.mat++;
    data.sptr++;
    data.qptr++;
    matchstemAddBranch(space, b, data.mat, data.mat, data.mis, data.ins,
                       data.del, data.child.a, data.child.b, data.parent.a,
                       data.parent.b);
  }
    ...
    while (data.qptr < m && data.child.a <= data.child.b) {
      ...
      if (data.l > data.sptr || data.l == 0) {
        if (data.l >= m + data.del - data.ins || data.l == 0) {
          kd_matchlcp(space, s, p, m, &data, cursufpos, m - data.del, seedlen,
                      vstack, maxdiff, sext, pmis, xoff, b);
          ...
          if (!kd_matchlcp(space, s, p, m, &data, cursufpos, lcplen, seedlen,
                           vstack, maxdiff, sext, pmis, xoff, b)) {
            break;
          }
        }
      } else {
        birth = getCharIntervalArr(space, s, data.child.a, data.child.b, 0,
                                   p[data.qptr]);
        if (data.kcnt < maxdiff || (isallowedIUPAC() && data.kcnt == maxdiff &&
                                    couldMatchIUPAC(p[data.qptr]))) {
...
inline void kdbest(void *space, Suffixarray *s, char *seqs[], Uint m, Uint sext,
                   Uint pmis, Uint xoff, Uint kp, matchstem_t *a[],
                   matchstem_t *b0[]) {

  Uint i, j, k, l, remainder, // remainderc,
      iprime, jprime;
  matchstem_t *b[2] = {NULL, NULL};
  // matchstem *c[2]={NULL, NULL};
  ...
  for (i = 0; i <= kp; i++) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      b[j] = kd_match(space, s, seqs[j], m, sext, pmis, xoff, i, iprime, jprime,
                      iprime, jprime, 0, 0);

      //     if(m > 25) {
      //      c[j] = kd_match(space, s, &seqs[j][m-26], 25, sext, pmis, xoff, i,
      //      iprime, jprime, iprime, jprime, 0, 0);
      //     }

      for (k = 1; k < b[j]->noofbranches; k++) {
        // full match with correct error bounds
        // --> store
        remainder = m - b[j]->branches[k].mat - b[j]->branches[k].mis -
                    b[j]->branches[k].ins;
        // fprintf(stderr, "branch: i=%d, j=%d, k=%d, mat=%d, mis=%d, ins=%d,
        // del=%d\n", 	i, j, k, b[j]->branches[k].mat, b[j]->branches[k].mis,
        //b[j]->branches[k].ins, b[j]->branches[k].del);
        if (b[j]->branches[k].mis + b[j]->branches[k].ins +
                b[j]->branches[k].del + remainder <=
            i) {
          ...
          a[j][0].branches =
              ALLOCMEMORY(space, NULL, branch_t, b[j]->noofbranches - 1);
          memmove(a[j][0].branches, &b[j]->branches[1],
                  sizeof(branch_t) * (b[j]->noofbranches - 1));
      ...
      /*
      if(m > 25) {
        for (k=1; k < c[j]->noofbranches; k++) {
          remainderc = m - c[j]->branches[k].mat - c[j]->branches[k].mis -
      b[j]->branches[k].ins;

          if (c[j]->branches[k].mis + c[j]->branches[k].ins +
      c[j]->branches[k].del + remainderc <= i){
            ...
            a[j][m-26].branches = ALLOCMEMORY(space, NULL, branch_t,
      c[j]->noofbranches-1); memmove(a[j][m-26].branches, &c[j]->branches[1],
      sizeof(branch_t) * (c[j]->noofbranches-1)); a[j][m-26].noofbranches =
      c[j]->noofbranches-1;
      ...
      if (i != kp) {
        ...
        b[j] = NULL;
        //        if(m > 25) {
        //          FREEMEMORY(space, c[j]->branches);
        //          FREEMEMORY(space, c[j]);
        //          c[j] = NULL;
        //        }
      }
    }
...
inline matchstem_t **kdbest2(void *space, Suffixarray *s, char *seqs[], Uint m,
                             Uint sext, Uint pmis, Uint xoff, Uint kp,
                             matchstem_t *a[], matchstem_t *b0[]) {

  Uint j, k, l, remainder, remainderc, iprime, jprime;
  matchstem_t *b[2] = {NULL, NULL};
  matchstem_t **c;
  unsigned char a0[2] = {a[0] == NULL, a[1] == NULL};

  c = ALLOCMEMORY(space, NULL, matchstem_t *, 2);
  c[0] = NULL;
  c[1] = NULL;
  ...
  for (j = 0; j < 2; j++) {
    ...
    b[j] = kd_match(space, s, seqs[j], m, sext, pmis, xoff, 0, iprime, jprime,
                    iprime, jprime, 0, 0);

    if (m > 25) {
      c[j] = kd_match(space, s, &seqs[j][m - 26], 25, sext, pmis, xoff, 0,
                      iprime, jprime, iprime, jprime, 0, 0);
    }
    ...
    if (m > 25) {
      for (k = 1; k < c[j]->noofbranches; k++) {
        ...
        if (c[j]->branches[k].mis + c[j]->branches[k].ins +
          ...
          a[j][m - 26].branches =
              ALLOCMEMORY(space, NULL, branch_t, c[j]->noofbranches - 1);
          memmove(a[j][m - 26].branches, &c[j]->branches[1],
                  sizeof(branch_t) * (c[j]->noofbranches - 1));
          a[j][m - 26].noofbranches = c[j]->noofbranches - 1;
...
inline matchstem_t *kdseeds(void *space, Suffixarray *s, char *p, Uint m,
                            Uint jump, Uint sext, Uint pmis, Uint xoff, Uint kp,
                            matchstem_t *b0, char nosuflinks) {
  Uint iprime, jprime, kprime, lprime, l, j, r, u, v, i, c = 0, q, ll;
  PairUint suflink;
  matchstem_t *a, *b;
  ...
  if (b0 == NULL) {
    // kp is set to zero here
    b = kd_match(space, s, p, m, sext, pmis, xoff, 0, iprime, jprime, iprime,
                 jprime, 0, 0);
  } else {
    b = b0;
  }
  ...
  if (b->noofbranches > 1) {
    ...
    memmove(a[0].branches, &b->branches[1],
            sizeof(branch_t) * (b->noofbranches - 1));
  ...
  for (i = jump; i < m; i += jump) {
    ...
    if (q == ll && q > jump && !nosuflinks) {
      ...
      if (q <= jump || getlcpval(s, u, v) <= jump || nosuflinks) {
        ...
        // match was stuck within interval -> take last completed interval and
        // jump iprime & jprime new interval kprime & lprime last completed
        iprime = u;
        jprime = v;
        for (j = 0; j < jump; j++) {
          suflink = getSuflink(s, iprime, jprime);
    ...
    assert(i < m);
    b = kd_match(space, s, &p[i], m - i, sext, pmis, xoff, kp, iprime, jprime,
                 kprime, lprime, c, c);
    ...
    if (b->noofbranches > 1) {
      ...
      memmove(a[i].branches, &b->branches[1],
              sizeof(branch_t) * (b->noofbranches - 1));
...
inline void dumpkdseeds(Suffixarray *s, matchstem_t *M, Uint m, char strand,
                        Uint T) {
  Uint i, j, k, h;
  ...
  for (i = 0; i < m; i++) {
    ...
    for (k = 0; k < M[i].noofbranches; k++) {
      ...
      if (M[i].branches[k].mat > 0 &&
          M[i].branches[k].r - M[i].branches[k].l <= T) {
        for (j = M[i].branches[k].l; j <= M[i].branches[k].r; j++) {
          printf("%u [", s->suftab[j]);
          char *cursuf = &s->seq->sequences[s->suftab[j]];
          for (h = 0; h < 20; h++) {
            printf("%c", cursuf[h]);
          }
          printf("]");
    ...
    // M[i].del, M[i].mis, M[i].l, M[i].r, strand);
    //  printf("%d:%d-(%d..%d)\t", i, M[i].mat, s->suftab[M[i].l],
    //  s->suftab[M[i].r]);
  }
}
...
inline branch_t *kmis(void *space, Suffixarray *s, char *P, Uint m, Uint k,
                      Uint *noofmatches) {
  ...
  data.l = 0;
  data.r = s->numofsuffixes - 1;
  child.a = data.l;
  child.b = data.r;
  data.u = 0;
  data.v = s->numofsuffixes - 1;
  data.p = 0;
  data.mis = 0;
    ...
    if (data.l < data.r) {
      while (1) {
        ...
        for (i = 0; i < bl_containerSize(c); i++) {
          ...
          if (data.l <= data.r) {
            if ((data.l != child.a || data.r != child.b) && data.mis + 1 <= k) {
              bl_vqueueEnqueue(&vqueue, &data);
              kmis_t *tmp =
                  bl_vqueueFrontN(&vqueue, bl_vqueueSize(&vqueue) - 1);
              tmp->p = lcp + 1;
              tmp->mis++;
            }
          }
...
void bl_kdMatchstemDestruct(void *space, matchstem_t *stem, Uint len) {
  Uint i;
  for (i = 0; i < len; i++) {
